...
//    @Query("select distinct new org.ssafy.d210.walk.dto.response.FriendRankingResponseDto(m.id, m.nickname, m.profileUrl, coalesce(e.streak, 0L))" +
...
//            " order by coalesce(e.streak, 0L) desc")
//    Slice<FriendRankingResponseDto> findStreakRankingByPage(Long myId, Pageable pageable, LocalDate yesterday);
//
//    @Query("select distinct new org.ssafy.d210.walk.dto.response.FriendRankingResponseDto(m.id, m.nickname, m.profileUrl, coalesce(sum(e.steps), 0L))" +
//            " from Exercise e left join Members m on e.member.id = m.id" +
...
//            " group by m.id" +
//            " order by coalesce(sum(e.steps), 0) desc")
